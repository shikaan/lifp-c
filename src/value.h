...
typedef Result(value_t *) result_clone_t;
...
static inline result_alloc_t valueCreate(arena_t *arena, value_type_t type) {
  value_t *value = nullptr;
  try(result_alloc_t, arenaAllocate(arena, sizeof(value_t)), value);
  value->type = type;

  if (value->type == VALUE_TYPE_LIST) {
    value_list_t *list = nullptr;
    try(result_alloc_t, listCreate(value_t, arena, VALUE_LIST_INITIAL_SIZE),
        list);
  ...
  if (value->type == VALUE_TYPE_CLOSURE) {
    ...
    node_t *form = nullptr;
    try(result_alloc_t, nodeCreate(arena, NODE_TYPE_LIST), form);
    value->value.closure.form = *form;
  }

  return ok(result_alloc_t, value);
};

static inline result_clone_t valueClone(arena_t *arena, const value_t *source) {
  value_t *destination = nullptr;
  try(result_clone_t, valueCreate(arena, source->type), destination);
  ...
  return ok(result_clone_t, destination);
}
